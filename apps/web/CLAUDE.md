...
Chesslet is a chess puzzle game built with Next.js 15. The unique twist: **every move must be a capture**, and the goal is to capture your own pieces until only one remains. The game uses a simplified 4x4 chess board with custom FEN notation.

This is the web app within a pnpm workspace monorepo. Game logic is shared via `@chesslet/game-logic` package.
