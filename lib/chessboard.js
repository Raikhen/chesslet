...
;(function () {
  ...
  var COLUMNS = ALPHABET.substring(0, BOARD_SIZE).split('')
  ...
  function validFen (fen) {
    ...
    // FEN should be n sections separated by slashes
    var chunks = fen.split('/')
    if (chunks.length !== BOARD_SIZE) return false

    // check each section
    for (var i = 0; i < BOARD_SIZE; i++) {
      if (chunks[i].length !== BOARD_SIZE ||
  ...
  window.squeezeFenEmptySquares = squeezeFenEmptySquares
  ...
  window.expandFenEmptySquares = expandFenEmptySquares
  ...
  function expandConfig (config) {
    ...
    if (!config.hasOwnProperty('pieceTheme') ||
      ...
      config.pieceTheme = '/img/chesspieces/staunty/{piece}.svg'
    }
  ...
  function constructor (containerElOrString, config) {
    ...
    widget.resize = function () {
      ...
      // set board width
      $board.css('width', squareSize * BOARD_SIZE + 'px')
